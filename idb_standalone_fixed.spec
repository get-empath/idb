...
# Manually add python.migrations files with correct paths
python_migrations_dir = os.path.join(project_root, 'python', 'migrations')
if os.path.exists(python_migrations_dir):
    # Add __init__.py files
    init_file = os.path.join(project_root, 'python', '__init__.py')
    if os.path.exists(init_file):
        datas.append((init_file, 'python'))
    
    migrations_init = os.path.join(python_migrations_dir, '__init__.py')
    if os.path.exists(migrations_init):
        datas.append((migrations_init, 'python/migrations'))
    
    # Add py310.py
    py310_file = os.path.join(python_migrations_dir, 'py310.py')
    if os.path.exists(py310_file):
        datas.append((py310_file, 'python/migrations'))
