...
from idb.grpc.idb_pb2 import VideoStreamRequest
...
class Client(ClientBase):
    ...
    async def stream_video(
        ...
        # Enhanced H.264 parameters
        keyframe_interval: int | None = None,
        h264_profile: str | None = None,
        max_bitrate: int | None = None,
        buffer_size: int | None = None,
        allow_frame_reordering: bool | None = None,
        realtime_optimization: bool | None = None,
        preset: str | None = None,
    ) -> AsyncGenerator[bytes, None]:
        self.logger.info("Starting connection to backend")
        
        # Map H.264 profile strings to protobuf enum values
        h264_profile_map = {
            "baseline": VideoStreamRequest.BASELINE,
            "main": VideoStreamRequest.MAIN,
            "high": VideoStreamRequest.HIGH,
        }
        
        # Map preset strings to protobuf enum values
        preset_map = {
            "custom": VideoStreamRequest.CUSTOM,
            "streaming": VideoStreamRequest.STREAMING,
            "low-latency": VideoStreamRequest.LOW_LATENCY,
            "high-quality": VideoStreamRequest.HIGH_QUALITY,
        }
        
        async with self.stub.video_stream.open() as stream:
            # Build the start request with enhanced parameters
            start_request = VideoStreamRequest.Start(
                file_path=output_file if self.is_local else None,
                fps=fps,
            ...
            # Add enhanced parameters if provided
            if keyframe_interval is not None:
                start_request.keyframe_interval = keyframe_interval
                self.logger.info(f"Enhanced H.264 - Keyframe interval: {keyframe_interval}")
            
            if h264_profile is not None and h264_profile in h264_profile_map:
                start_request.h264_profile = h264_profile_map[h264_profile]
                self.logger.info(f"Enhanced H.264 - Profile: {h264_profile}")
            
            if max_bitrate is not None:
                start_request.max_bitrate = max_bitrate
                self.logger.info(f"Enhanced H.264 - Max bitrate: {max_bitrate} kbps")
            
            if buffer_size is not None:
                start_request.buffer_size = buffer_size
                self.logger.info(f"Enhanced H.264 - Buffer size: {buffer_size} kbps")
            
            if allow_frame_reordering is not None:
                start_request.allow_frame_reordering = allow_frame_reordering
                self.logger.info(f"Enhanced H.264 - Allow frame reordering: {allow_frame_reordering}")
            
            if realtime_optimization is not None:
                start_request.realtime_optimization = realtime_optimization
                self.logger.info(f"Enhanced H.264 - Real-time optimization: {realtime_optimization}")
            
            if preset is not None and preset in preset_map:
                start_request.preset = preset_map[preset]
                self.logger.info(f"Enhanced H.264 - Preset: {preset}")
            
            # Send the request
            if self.is_local and output_file:
                self.logger.info(f"Streaming locally with companion writing to {output_file}")
            else:
                self.logger.info("Starting streaming over the wire")
            
            await stream.send_message(VideoStreamRequest(start=start_request))
